from _main_.utils.commons import (
    is_value,
    parse_bool,
    parse_dict,
    parse_int,
...
class Validator:
    ...
    def expect_one_of(self, lst):
        for (field_name, field_type) in lst:
            self.fields[field_name] = {"type": field_type, "is_required": False}
        return self
    ...
    def expect_all(self, lst):
        for (field_name, field_type, is_required) in lst:
            self.fields[field_name] = {"type": field_type, "is_required": is_required}
        return self
    ...
    def verify(self, args, strict=False):
        try:
            ...
            self.fields = {}
            self.rename_fields = set()
            print("Error in validator: ", e)
            return None, CustomError(e)
