...
def staff_list(request):
    ...
    for s in page_obj:
        socials_str = "||".join(
            [f"{key}#{value}" for key, value in s.user.info.get("socials", {}).items()]
        )
...
def update_staff(request, id):
    ...
    if request.method == "POST":
        ...
        facebook = request.POST.get(
            "facebook", staff_member.user.info.get("socials", {}).get("facebook")
        )
        linkedin = request.POST.get(
            "linkedin", staff_member.user.info.get("socials", {}).get("linkedin")
        )
        twitter = request.POST.get(
            "twitter", staff_member.user.info.get("socials", {}).get("twitter")
        )
