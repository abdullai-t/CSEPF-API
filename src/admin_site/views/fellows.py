...
def fellow_list(request):
    ...
    for f in page_obj:
        socials_str = "||".join(
            [f"{key}#{value}" for key, value in f.user.info.get("socials", {}).items()]
        )
        f.socials = socials_str
        f.user_image_url = f.user.picture.url if f.user.picture else ""

    return render(
        request, "fellows_list.html", {"page_obj": page_obj, "schools": SCHOOLS}
    )
...
def add_fellow(request):
    if request.method == "POST":
        ...
        user.info = {
            "socials": {"facebook": facebook, "linkedin": linkedin, "twitter": twitter}
        }
        user.save()
...
def update_fellow(request, id):
    ...
    if request.method == "POST":
        facebook = request.POST.get(
            "facebook", fellow.user.info.get("socials", {}).get("facebook")
        )
        linkedin = request.POST.get(
            "linkedin", fellow.user.info.get("socials", {}).get("linkedin")
        )
        twitter = request.POST.get(
            "twitter", fellow.user.info.get("socials", {}).get("twitter")
        )
